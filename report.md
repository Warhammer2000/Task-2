...
Lovable hosting does not expose UA-aware middleware or build-time prerendering, so patterns 1–3 in the brief are not feasible. I implemented pattern 4: a public Supabase edge function `og-event` (`/functions/v1/og-event/<eventId>`, `verify_jwt = false`) that fetches the event via the anon client (RLS still applies — only published+public events leak metadata; unlisted/draft fall through to a generic "event not found" page), then returns SSR HTML with per-event `og:title`, `og:description` (truncated to 160 chars), `og:image` (cover URL with `/placeholder.svg` fallback), `og:url`, `og:type=event`, and the mirrored `twitter:*` tags. Humans get a `<meta http-equiv="refresh">` plus `window.location.replace` that bounces them to `/events/:id` (the SPA route is untouched, so RSVP/gallery/feedback/reports/check-in/CSV all keep working). The event detail page now has a "share" button that copies this share URL — paste it in Slack/Twitter/Telegram and the unfurl shows the event title + image. Verify with `curl -A "Twitterbot/1.0" https://<project>.supabase.co/functions/v1/og-event/<eventId> | grep -i og:` (per-event values), the Twitter Card Validator, and the Facebook Sharing Debugger.
